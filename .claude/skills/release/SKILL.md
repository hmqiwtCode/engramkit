...
- **The wheel bundles `engramkit/dashboard_static/`, but the directory is gitignored.** CI/release workflows rebuild it from `dashboard/` source on every run via `./scripts/build-dashboard.sh` — the only producer. Never commit anything under `engramkit/dashboard_static/`. If you see it staged, run `git restore --staged engramkit/dashboard_static/` before continuing.
...
The bundled dashboard rebuild is handled by CI/release workflows; nothing under `engramkit/dashboard_static/` should ever be staged.
